...
*/
        ...
        internal string AutoChangeFloatingPoint(double dVal, GraphPane pane)
        {
            if (IsLog)
            {
                if (dVal >= 1.0)
                ...
                else if (dVal < 1.0)
                {
                    ...
                    DataToFormat(dVal);
                //return DataToFormat(dVal);
            }
            return Scale.Default.Format;
        }
        private string DataToFormat(double data)
        {
            string szFmt = "f";
            double data1 = Math.Abs(data);

            if (data1 >= 1.0E-3)
            {
                szFmt = "f3";
            }
            else if (data1 >= 1.0E-4)
            {
                szFmt = "f4";
            }
            else if (data1 >= 1.0E-5)
            {
                szFmt = "f5";
            }
            else if (data1 >= 1.0E-6)
            {
                szFmt = "f6";
            }
            else if (data1 >= 1.0E-7)
            {
                szFmt = "f7";
            }
            else if (data1 >= 1.0E-8)
            {
                szFmt = "f8";
            }
            else if (data1 >= 1.0E-9)
            {
                szFmt = "f9";
            }
            else if (data1 >= 1.0E-10)
            {
                szFmt = "f10";
            }
            else if (data1 >= 1.0E-11)
            {
                szFmt = "f11";
            }
            else if (data1 >= 1.0E-12)
            {
                szFmt = "f12";
            }
            return szFmt;
...
//			using ( Pen gridPen = grid.GetPen( pane, scaleFactor ) )
			{
                ...
                float fMaxLabelPixal=0;
                //Draw Min Max Label
                if (isShowMinMaxLabel && this._ownerAxis is XAxis)
                {
                    ...
                    //panxb: call make label to format the label text
                    string minLabel, maxLabel;
                    minLabel = maxLabel = string.Empty;

                    if (this is LogScale)
                    {
                        double _min1 = Math.Log10(_min);
                        double _max1 = Math.Log10(_max);
                        minLabel = MakeLabel(pane, 0, _min1);
                        maxLabel = MakeLabel(pane, 0, _max1);
                    }
                    else
                    {
                        minLabel = MakeLabel(pane, 0, _min);
                        maxLabel = MakeLabel(pane, 0, _max);
                    }

                    float fMinLabelPixal = LocalTransform(_minLinTemp) + _fontSpec.GetWidth(g, minLabel, scaleFactor) / 2; //fix #34299

                    if (this._maxPix - this._minPix > maxLabelSize.Width)
                    {
                        DrawMinMaxLabel(g, pane, minLabel, fMinLabelPixal, shift, maxSpace, scaledTic, charHeight, scaleFactor);
                        lastPixVal = fMinLabelPixal;
                    }

                    fMaxLabelPixal = LocalTransform(_maxLinTemp); //- _fontSpec.GetWidth(g, maxLabel, scaleFactor) / 2;

                    if (Math.Abs(fMinLabelPixal - fMaxLabelPixal) > maxLabelSize.Width + 10.0f)
                       DrawMinMaxLabel(g, pane, maxLabel, fMaxLabelPixal, shift, maxSpace, scaledTic, charHeight, scaleFactor);
...
//					grid.Draw( g, gridPen, pixVal2, topPix );
					...
					if ( _isVisible && !isSkipZone )
					{
						...
						if ( IsPreventLabelOverlap &&
							...
							continue;
                        if (IsPreventLabelOverlap && isShowMinMaxLabel && 
                            this._ownerAxis is XAxis && Math.Abs(pixVal - fMaxLabelPixal) < maxLabelSize.Width)
                            continue;
        ...
        private bool isShowMinMaxLabel = true;
        public bool ShowMinMaxLabel
        {
            get { return isShowMinMaxLabel; }
            set { isShowMinMaxLabel = value; }
        }
        internal void DrawMinMaxLabel(Graphics g, GraphPane pane, string tmpStr, float pixVal,
                       float shift, float maxSpace, float scaledTic, float charHeight, float scaleFactor)
        {
            ...
            if (_ownerAxis.MajorTic.IsOutside)
                ...
                textTop = charHeight * _labelGap + 0.5f;

            // draw the label
            //string tmpStr = MakeLabel( pane, i, dVal );
            //string tmpStr = ((long)(dValue)).ToString();// this.MakeLabel(pane, 0, dValue);// dValue.ToString("f2");
            //if (_isUseUnitForLabel)
            //{
            //    double scaleMult = Math.Pow((double)10.0, dVal);
            //    tmpStr = _ownerAxis.MakeLabelEventWorks(pane, i, scaleMult); ;
            //}
            //else
            //    tmpStr = _ownerAxis.MakeLabelEventWorks(pane, i, dVal);
