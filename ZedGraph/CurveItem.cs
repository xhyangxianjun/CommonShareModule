...
*/
	...
	#region Utility Methods
        ...
        public PointPair GetNearestPointPair(double x)
        {
            if (this.IsLine == false || this.Points.Count==0)
                return null;
            PointPair tempPP = null;
            for(int i=1;i<Points.Count-1;i++)
            {
                if (Points[i].X == PointPair.Missing)
                    continue;
                if (Points[i].X == x)
                {
                    tempPP = Points[i];
                ...
                else if(Points[i].X < x && x<= Points[i + 1].X)
                {
                    if((x-Points[i].X)>(Points[i+1].X-x))
                    { tempPP = Points[i + 1];break; }
                    else
                    {
                        tempPP = Points[i];break;
                    }
                }
                 else if(Points[i-1].X <= x && x < Points[i].X)
                {
                    if ((x - Points[i-1].X) > (Points[i].X - x))
                    { tempPP = Points[i]; break; }
                    else
                    {
                        tempPP = Points[i-1]; break;
        ...
        public PointPair GetMaxYPointPair()
        {
            if (this.IsLine == false || this.Points.Count == 0)
                return null;
            PointPair tempPP = null;
            ...
            {
                if (Points[i].X == PointPair.Missing || Points[i].Y == PointPair.Missing)
                    continue;
                if (tempPP == null)
                {
                    tempPP = Points[i];
